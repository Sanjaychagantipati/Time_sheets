  ...
  return (
    <div className="w-full max-w-7xl mx-auto flex flex-col gap-6 animate-fade-in text-white pb-10 px-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <div className="bg-[#111111] border border-[#2A2A2A] p-5 rounded-2xl shadow-xl flex flex-col gap-1.5 text-center sm:text-left transition-all duration-300 hover:border-[#FF7A00]/20">
          <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">Today's First In</span>
          <span className="text-xl font-extrabold text-white font-mono mt-1">
            {todayLog ? formatTime12h(todayLog.clockIn) : '--:--'}
          </span>
        </div>
        <div className="bg-[#111111] border border-[#2A2A2A] p-5 rounded-2xl shadow-xl flex flex-col gap-1.5 text-center sm:text-left transition-all duration-300 hover:border-[#FF7A00]/20">
          <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">Today's Last Out</span>
          <span className="text-xl font-extrabold text-white font-mono mt-1">
            {todayLog && todayLog.clockOut && !isClockedIn ? formatTime12h(todayLog.clockOut) : '--:--'}
          </span>
        </div>
        <div className="bg-[#111111] border border-[#2A2A2A] p-5 rounded-2xl shadow-xl flex flex-col gap-1.5 text-center sm:text-left transition-all duration-300 hover:border-[#FF7A00]/20">
          <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">Today's Total Hours</span>
          <span className="text-xl font-black text-[#FF7A00] font-mono mt-1">
            {todayLog ? `${todayLog.hours.toFixed(2)} hrs` : '0.00 hrs'}
          </span>
        </div>
      </div>

      {/* Row 3: Clock Button Card & Attendance History (Side-by-side on desktop, stacked on mobile) */}
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 items-start">
        
        {/* Clock Button Card (2 columns on desktop) */}
        <div className="lg:col-span-2 w-full">
          <ClockCard onShiftLogged={refreshData} setToast={setToast} />
        </div>

        {/* Attendance History Table (3 columns on desktop) */}
        <div className="lg:col-span-3 w-full bg-[#111111] border border-[#2A2A2A] rounded-2xl overflow-hidden shadow-2xl transition-all duration-300 hover:border-[#FF7A00]/10">
